...
def get_historic_df(df: df, numerator_id: str, breakdown: str, status: str, db_source: str, rp_enddate: str, month_int=12):
  ##accounts for change from STP to ICB in historic data
  if "STP" in breakdown:
    breakdown = breakdown.replace("STP", "ICB")
  ...
  historic_df = (
    ...
    .filter(
      (F.col("MEASURE_ID") == numerator_id)
      & (F.col("BREAKDOWN") == breakdown)
      & (F.col("STATUS") == "Final") ###historic data based on Final data submission
      & (F.col("SOURCE_DB") == db_source)
...
def get_percentage_change_df(numerator_df: df, historic_df: df) -> df:
  percent_change_prep_df = (
    numerator_df
    .join(historic_df,
                  (numerator_df.PRIMARY_LEVEL == historic_df.PRIMARY_LEVEL) &   
                  (numerator_df.SECONDARY_LEVEL == historic_df.SECONDARY_LEVEL),
                  how="left")
    .select(
      numerator_df.BREAKDOWN, 
      numerator_df.PRIMARY_LEVEL, numerator_df.PRIMARY_LEVEL_DESCRIPTION,
      numerator_df.SECONDARY_LEVEL, numerator_df.SECONDARY_LEVEL_DESCRIPTION,
      numerator_df.NUMERATOR_COUNT,
      historic_df.HISTORIC_COUNT
    )
  )
  
  return percent_change_prep_df
 
def produce_percentage_change_agg_df(
  db_output: str,
  db_source: str,
  table_name: str,
  filter_clause: Column,
...
def produce_4ww_agg_df(
  ...
  column_order: list
    ...
    Where the breakdown includes Pathways, the defined preparation table is updated to the 
    master table containing Pathway breakdowns.
    
    NOTE: All required filtering should be done on the final prep table
    """  
    prep_df = spark.table(f"{db_output}.{table_name}")
    
    if "Pathway" in breakdown_name:
      prep_df = spark.table(f"{db_output}.{table_name}_pathway")
...
def produce_date_filter_4ww_agg_df(
  db_output: str,  
  db_source: str,
  table_name: str,
  filter_clause: Column,
...
# DBTITLE 1,Direct Age and Gender Standardised Rate functions
...
def mhsds_suppression(df: df, suppression_type: str, breakdown: str, measure_id: str, rp_enddate: str, status: str, numerator_id: str, db_source: str) -> df:
  ...
  if child_breakdown(breakdown) == " Mean Deviation of Upper Ethnicity" or "Standardised Rate Difference from" in breakdown:
    ...
    if suppression_type == "count":
    ...
    elif suppression_type == "percent_change":         
      perc_values = (
        df
        .filter(
          (F.col("MEASURE_ID") == measure_id)
      ...
      historic_values = (
        ...
        .filter(
          (F.col("MEASURE_ID") == numerator_id)
          & (F.col("BREAKDOWN") == breakdown)
          & (F.col("REPORTING_PERIOD_END") == dt2str(add_months(str2dt(rp_enddate), -12)))
          & (F.col("STATUS") == "Final")
          & (F.col("SOURCE_DB") == db_source)
        )        
        .select(
          F.col("BREAKDOWN"), F.col("PRIMARY_LEVEL"), F.col("SECONDARY_LEVEL"), F.col("MEASURE_ID").alias("HISTORIC_ID"), F.col("MEASURE_VALUE").alias("HISTORIC_VALUE")
        )
      )
 
      perc_num_comb = (
        num_values.alias("num")
        .join(perc_values.alias("perc"), ["BREAKDOWN", "PRIMARY_LEVEL", "SECONDARY_LEVEL"])
        .join(historic_values.alias("hist"), ["PRIMARY_LEVEL", "SECONDARY_LEVEL"])
        .select(
          F.col("perc.*"),
          F.col("num.NUMERATOR_ID"), F.col("num.NUMERATOR_VALUE"),
          F.col("hist.HISTORIC_ID"), F.col("hist.HISTORIC_VALUE"),
        )
      )
 
      #percentage change suppression logic
      perc_change_supp_logic = (
      F.when(
        (F.col("NUMERATOR_VALUE_SUPP") == "*") | (F.col("HISTORIC_VALUE_SUPP") == "*"),
        F.lit("*")
      )
       .otherwise(F.round(F.col("MEASURE_VALUE"), 0)) #round to nearest whole number (0dp)
      )
      ...
      supp_df = (
        perc_num_comb
        .withColumn("NUMERATOR_VALUE_SUPP", supp_method(F.col("NUMERATOR_VALUE")))
        .withColumn("HISTORIC_VALUE_SUPP", supp_method(F.col("HISTORIC_VALUE")))
        .withColumn("MEASURE_VALUE", perc_change_supp_logic)
      )
      
 
    elif suppression_type == "percent":
      perc_values = (
        df
        .filter(
          (F.col("MEASURE_ID") == measure_id)
